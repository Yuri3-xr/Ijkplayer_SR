...
static int queue_picture(FFPlayer *ffp, AVFrame *src_frame, double pts, double duration, int64_t pos, int serial)
{
	...
	/* ADD: x4 SR recommand less or equal than 360P on SnapDragon 865 SoC */
	int sr_switch = 1;
	if (src_frame->height > 360) {
		sr_switch = 0;
	}
    ...
    if (!vp->bmp || !vp->allocated ||
        ...
        vp->allocated = 0;
		/* ADD: x4 SR width and heigh */
        if (sr_switch) {
            if (vp->width != src_frame->width*4 || vp->height != src_frame->height*4)
                ffp_notify_msg3(ffp, FFP_MSG_VIDEO_SIZE_CHANGED, src_frame->width*4, src_frame->height*4);

			vp->width = src_frame->width * 4;
			vp->height = src_frame->height * 4;
			vp->format = AV_PIX_FMT_0BGR32;
            // ffp_notify_msg3(ffp, FFP_MSG_VIDEO_SIZE_CHANGED, vp->width, vp->height);

			/* the allocation must be done in the main thread to avoid
			locking problems. */
			alloc_picture(ffp, AV_PIX_FMT_0BGR32);
		} else {
    ...
    if (vp->bmp) {
		// ADD: do SR inference here
		if (sr_switch) {
			uint8_t *dst_data[4];
			int dst_linesize[4];
			const int *src_linesize = (const int*)src_frame->linesize;

            AVFrame *m_inFrame = av_frame_alloc();      // 图像格式转换后的frame
            m_inFrame->width = src_frame->width;
            m_inFrame->height = src_frame->height;
            m_inFrame->format = AV_PIX_FMT_0BGR32;
            if (av_frame_get_buffer(m_inFrame, 0) < 0 ||
                av_frame_make_writable(m_inFrame) < 0)
                return false;
			// 拷贝元数据
            av_frame_copy_props(m_inFrame, src_frame);
            int result=ijk_image_convert(src_frame->width, src_frame->height, AV_PIX_FMT_0BGR32, m_inFrame->data, m_inFrame->linesize, src_frame->format, (const uint8_t**)src_frame->data, src_linesize);
            av_log(NULL, AV_LOG_FATAL, "ijk_image_convert result %d\n", result); 

			int bigendian_switch;
			if (AV_PIX_FMT_0BGR32 == AV_PIX_FMT_0BGR) {
				bigendian_switch = 1;
			}
			else {
				bigendian_switch = 0;
			...
			xjtu_get_sr_result(m_inFrame, bigendian_switch);
            av_log(NULL, AV_LOG_FATAL, "src_frame buf 0x%x\n", src_frame->buf[0]);  
            // 释放
            av_frame_unref(src_frame);
            av_log(NULL, AV_LOG_FATAL, "src_frame unref buf 0x%x\n", src_frame->buf[0]);  
            av_frame_ref(src_frame, m_inFrame);
            av_log(NULL, AV_LOG_FATAL, "src_frame ref buf 0x%x\n", src_frame->buf[0]);  
            // av_frame_unref(m_inFrame); // 外部调用玩释放
...
static int ffplay_video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        while (ret >= 0) {
            ...
            pts = (frame->pts == AV_NOPTS_VALUE) ? NAN : frame->pts * av_q2d(tb);
            av_log(NULL, AV_LOG_FATAL, "frame buf 0x%x\n", frame->buf[0]);  
            ret = queue_picture(ffp, frame, pts, duration, frame->pkt_pos, is->viddec.pkt_serial);
            av_log(NULL, AV_LOG_FATAL, "frame ref buf 0x%x\n", frame->buf[0]);
            av_frame_unref(frame);
            av_log(NULL, AV_LOG_FATAL, "frame unref buf 0x%x\n", frame->buf[0]);
#if CONFIG_AVFILTER
        }
#endif
